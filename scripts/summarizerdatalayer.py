...
class SummarizerDataLayer(ISummarizer):
    ...
    def get_Results(self,name="default") -> 'dict':
        '''
        @desc
        ...
        '''
        # adding saved parameters
        data = pd.DataFrame([self.__results])
        data.to_csv(os.path.join(os.getcwd(),"results","iot",name+".csv"),index=False)
